...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const config = await loadProductConfig("channel-letters--faux-neon", locale);
  return {
    title: config.seo.title,
    description: config.seo.metaDescription,
    keywords: config.seo.keywords,
    alternates: getAlternates("/products/channel-letters/faux-neon"),
  };
}
...
export default async function FauxNeonPage() {
  ...
  const jsonLd = {
    ...
    additionalProperty: [{ "@type": "PropertyValue", name: "Model", value: "EdgeLuxe LP 11-N" }],
    ...
    <>
      ...
      <section className="relative bg-bg-primary overflow-hidden">
        ...
        <div className="relative z-10 container-max section-padding pt-32 md:pt-36">
          <Breadcrumbs locale={locale} items={[{ name: t(locale, "breadcrumbs.home"), href: "/" }, { name: "Products", href: "/products" }, { name: "Channel Letters", href: "/products/channel-letters" }, { name: "Faux Neon" }]} />
          <AnimatedSection>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              ...
              <BeforeAfterSlider daySrc="/products/faux-neon-day.jpg" nightSrc="/products/faux-neon-night.jpg" alt="Faux neon channel letters — illuminated storefront, night shot" />
            </div>
          </AnimatedSection>
        </div>
      </section>
