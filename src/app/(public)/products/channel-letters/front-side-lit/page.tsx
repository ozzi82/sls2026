...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const config = await loadProductConfig("channel-letters--front-side-lit", locale);
  return {
    title: config.seo.title,
    description: config.seo.metaDescription,
    keywords: config.seo.keywords,
    alternates: getAlternates("/products/channel-letters/front-side-lit"),
  };
}
...
export default async function FrontSideLitPage() {
  ...
  const jsonLd = {
    ...
    additionalProperty: [{ "@type": "PropertyValue", name: "model", value: "EdgeLuxe LP 11-FS" }],
    ...
    <>
      ...
      <section className="relative bg-bg-primary overflow-hidden">
        ...
        <div className="relative z-10 container-max section-padding pt-32 md:pt-36">
          <Breadcrumbs locale={locale} items={[{ name: t(locale, "breadcrumbs.home"), href: "/" }, { name: "Products", href: "/products" }, { name: "Channel Letters", href: "/products/channel-letters" }, { name: "Front Side Lit" }]} />
          <AnimatedSection>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              ...
              <BeforeAfterSlider daySrc="/products/face-side-day.jpg" nightSrc="/products/face-side-night.jpg" alt="Front side lit channel letters — forward-facing side glow, night shot" />
            </div>
          </AnimatedSection>
        </div>
      </section>
      ...
      <section className="section-padding bg-bg-light">
        <div className="container-max">
          <AnimatedSection><div className="text-center mb-16"><div className="gold-line mx-auto mb-6" /><h2 className="text-3xl md:text-4xl font-heading font-bold text-text-dark mb-4"><SafeHtml html={featuresData.heading} /></h2><p className="text-text-dark/60 max-w-xl mx-auto">Every front side lit channel letter set is manufactured to the same exacting standards that define the Sunlite brand. Wholesale direct to your shop.</p></div></AnimatedSection>
        ...
        <div className="container-max">
          <AnimatedSection>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              ...
              <div className="relative aspect-[4/3] rounded-xl overflow-hidden"><Image src="/products/face-side-day.jpg" alt="Front side lit channel letters — retail installation, daytime" fill sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover" /></div>
            </div>
          </AnimatedSection>
        </div>
      </section>
