...
export default function QualityProcessPage() {
  ...
  const metricsData = getBlock("quality-metrics")!.data as any;
  const stepsData = getBlock("process-steps")!.data as any;
  const materialsData = getBlock("materials")!.data as any;
  const benefitsData = getBlock("trade-benefits")!.data as any;
      ...
      <section className="bg-bg-navy border-t border-white/10 border-b border-b-white/10">
        <div className="container-max px-4 sm:px-6 lg:px-8 py-10">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {metricsData.items.map((metric: any, index: number) => (
        ...
        <div className="container-max">
          <AnimatedSection>
            <div className="text-center mb-16">
              ...
              <h2 className="text-3xl md:text-4xl font-heading font-bold text-text-dark mb-4">{stepsData.heading}</h2>
              <p className="text-text-dark/60 max-w-2xl mx-auto">{stepsData.description}</p>
            </div>
          </AnimatedSection>
      ...
      <section className="section-padding bg-bg-primary">
        <div className="container-max">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <AnimatedSection>
              <div className="gold-line mb-6" />
              <h2 className="text-3xl md:text-4xl font-heading font-bold text-white mb-6">{materialsData.heading}</h2>
              <p className="text-white/60 leading-relaxed mb-6">{materialsData.content}</p>
            ...
            <AnimatedSection delay={0.15}>
              <PlaceholderImage label={materialsData.image} className="rounded-xl" aspectRatio="aspect-[4/3]" />
            </AnimatedSection>
          </div>
        </div>
      </section>
        ...
        <div className="container-max">
          <AnimatedSection>
            <div className="max-w-3xl mx-auto text-center">
              <div className="gold-line mx-auto mb-6" />
              <h2 className="text-3xl md:text-4xl font-heading font-bold text-white mb-6">{benefitsData.heading}</h2>
              <p className="text-white/60 leading-relaxed mb-10">{benefitsData.description}</p>
            </div>
          </AnimatedSection>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {benefitsData.items.map((item: any, index: number) => (
